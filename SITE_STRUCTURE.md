...
グローバルナビゲーションは `トップ / 任意加入 / 立場別 / 学校とPTA / 全国調査 / 教育委員会回答 / 論考・資料 / 教育委員会へ提出` を基本系統とします。`任意加入` の実務入口は `membership.html` に統一し、詳細解説 `pta-membership-optin.html` は入口から参照する補助ページとして扱います。デスクトップ・モバイルともHTMLへ静的に出力し、`js/site.js` は現在地表示・ドロップダウン開閉・検索等の補助動作だけを担当します。
